...
tf.app.flags.DEFINE_boolean ('use_native_decoder', True, 'use the libctc_decoder_with_kenlm.so library containing the decoder implementation.')
...
def create_inference_graph(batch_size=None, output_is_logits=False, use_new_decoder=FLAGS.use_native_decoder):
...
def do_single_file_inference(input_file_path):
    with tf.Session(config=session_config) as session:
        inputs, outputs = create_inference_graph(batch_size=1, use_new_decoder=FLAGS.use_native_decoder)
